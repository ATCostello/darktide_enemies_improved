...
local temp_position_offset = { 0, 0, 0 }
...
template.on_enter = function(widget, marker, template)
	...
	temp_position_offset[1] = 0
	temp_position_offset[2] = 0
	temp_position_offset[3] = fs.hb_y_offset
	template.position_offset = temp_position_offset
	...
	-- skip horde enemies
	if breed_type == "horde" and not fs.horde_enable and not fs.horde_clusters_enable then
		marker.draw = false
		marker.remove = true
		return
	end
	...
	if content.breed and is_alive then
		...
		if not marker.world_position then
			...
			local prev = marker._last_pos

			if not prev or Vector3.distance_squared(prev, root_position) > 0.01 then
				marker.world_position:store(root_position)
				marker._last_pos = root_position
			end
		end
	end
end
...
template.update_function = function(parent, ui_renderer, widget, marker, template, dt, t)
	...
	if not unit then
		...
		mod.latest_damaged_enemies[unit] = nil
		marker.remove = true
		return
	end
	...
	if cluster and fs.horde_clusters_enable and fs.healthbar_enable then
		...
		if t >= next_cluster_update then
			...
			for i = 1, unit_count do
				...
				if entry then
					...
					if he and mod.detect_alive(u) then
						local ch = he:current_health() or 0
						local mh = he:max_health() or 0
						total_current = total_current + ch
						total_max_instant = total_max_instant + mh
					end
				end
			end
		...
		if content.breed and is_alive then
			...
			if not marker.world_position then
				...
				if not prev or Vector3.distance_squared(prev, root_position) > 0.05 then
					marker.world_position:store(root_position)
					marker._last_pos = root_position
				end
			end
	...
	if damage_taken_since_last > 0 and health_extension and not is_dead then
		...
		if show_damage_number then
			if fs.hb_damage_show_only_latest then
				...
				mod.latest_damaged_lookup[unit] = true

				-- remove oldest entries if we exceed the limit
				while #mod.latest_damaged_enemies > fs.hb_damage_show_only_latest_value do
					local removed = table.remove(mod.latest_damaged_enemies, 1)
					if removed then
						mod.latest_damaged_lookup[removed] = nil
					end
				end

				-- cleanup dead units
				for i = #mod.latest_damaged_enemies, 1, -1 do
					local u = mod.latest_damaged_enemies[i]
					if not Unit.alive(u) then
						table.remove(mod.latest_damaged_enemies, i)
					end
				end
			end
			...
			if fs.show_damage_numbers or fs.hb_text_show_damage then
				if content.add_on_next_number or was_critical or should_add then
					...
					if content.last_hit_world_position then
						local pos = content.last_hit_world_position:unbox()

						if not damage_number.hit_world_position then
							damage_number.hit_world_position = Vector3Box(pos)
						else
							damage_number.hit_world_position:store(pos)
						end
					end
				else
	...
	if fs.hb_damage_show_only_latest then
		if not mod.latest_damaged_lookup[unit] then
			marker.draw = false
		end
	end
